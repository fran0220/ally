...
export default function LLMStageStreamCard({
  ...
  const resolveProgressText = useCallback(
    (value: string | undefined, fallbackKey: string): string => {
  ...
  const statusLabel = useCallback(
    (status: LLMStageViewStatus): string => {
  ...
  useEffect(() => {
    ...
    if (renderFrameRef.current != null) return;
    renderFrameRef.current = requestAnimationFrame(renderNextFrame);
  }, [outputText, showCursor, outputStageId, activeStageId, smoothStreaming, renderNextFrame, stopRenderLoop]);
    ...
    <article className="glass-surface-modal flex h-full w-full flex-col overflow-hidden rounded-[var(--glass-radius-xl)] text-[var(--glass-text-primary)]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 p-5 md:grid-cols-[17rem_1fr] md:gap-5 md:p-6">
        ...
        <section className="glass-surface-soft min-h-[320px] rounded-[var(--glass-radius-lg)] border border-[var(--glass-stroke-base)]">
          ...
          <div ref={outputRef} className="h-[52vh] overflow-y-auto px-4 py-4">
                    ...
                    <pre className="min-h-[110px] whitespace-pre-wrap break-words px-3 py-3 font-mono text-[14px] leading-7 text-[var(--glass-text-secondary)]">
                      {structuredOutput.reasoning ||
                        (structuredOutput.finalText
                          ? t('stageCard.reasoningNotProvided')
                          : t('stageCard.waitingModelOutput'))}
                      {showCursor && !structuredOutput.finalText ? (
