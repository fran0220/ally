...
interface ProjectAssetsCharacter {
  id: string;
  name: string;
...
interface ProjectAssetsResponse {
  characters: ProjectAssetsCharacter[];
}
...
export function CharacterCreationModal({
  ...
  const projectAssetsQuery = useQuery({
    ...
    queryFn: () => apiRequest<ProjectAssetsResponse>(`/api/novel-promotion/${projectId}/assets`),
    enabled: mode === 'project' && !!projectId,
  });

  const availableCharacters = useMemo(() => {
    if (mode !== 'project') return [];
    const items = projectAssetsQuery.data?.characters || [];
    return items.map((c) => ({
      id: c.id,
      name: c.name,
      appearances: c.appearances || [],
    }));
  }, [mode, projectAssetsQuery.data?.characters]);
