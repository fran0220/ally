...
import { GlassButton, GlassChip, GlassSurface } from '../primitives';
import PanelEditFormV2 from './PanelEditFormV2';
import type { PanelEditData } from './PanelEditFormV2';
      ...
      <div className="relative">
        <div className="aspect-[9/16] w-full overflow-hidden bg-[rgba(255,255,255,0.35)]">
          {isDeleting || isModifying || isTaskRunning ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
              ...
              <GlassButton size="sm" variant="ghost" onClick={onClearError}>
                {t('common.cancel')}
              </GlassButton>
            </div>
          ) : selectedCandidate ? (
        ...
        <div className="absolute bottom-2 left-2 right-2 flex flex-wrap items-center gap-2">
          <GlassButton
            size="sm"
            variant="secondary"
            onClick={() => onRegeneratePanelImage(panel.id, 1, isTaskRunning)}
          >
            {t('image.regenerate')}
          </GlassButton>
          <GlassButton size="sm" variant="secondary" onClick={onOpenEditModal}>
            {t('image.editImage')}
          </GlassButton>
          <GlassButton size="sm" variant="secondary" onClick={onOpenAIDataModal}>
            {t('aiData.title')}
          </GlassButton>

          {candidateData ? (
            <>
              <GlassButton size="sm" variant="ghost" onClick={() => onCancelCandidate(panel.id)}>
                ...
                {candidateData.candidates.slice(0, 4).map((_, index) => (
                  <button
                    ...
                    className={`h-2.5 w-2.5 rounded-full ${
                      index === candidateData.selectedIndex
                        ? 'bg-[var(--glass-accent-from)]'
                        : 'bg-[var(--glass-bg-surface)]/80 border border-[var(--glass-stroke-base)]'
                    }`}
