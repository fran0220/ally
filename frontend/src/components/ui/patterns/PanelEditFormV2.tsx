...
import { GlassChip, GlassField, GlassInput, GlassTextarea } from '../primitives';
    ...
    <div className={`ui-pattern-form ui-pattern-form-${uiMode} space-y-2`}>
      ...
      {saveStatus === 'error' ? (
        <div className="flex flex-wrap items-center gap-2">
          <GlassChip tone="danger">{saveErrorMessage || t('common.saveFailed')}</GlassChip>
          {onRetrySave ? (
            <button
              type="button"
              onClick={onRetrySave}
        ...
        <GlassField label={t('panel.sourceText')}>
          <div className="rounded-[var(--glass-radius-md)] bg-[var(--glass-bg-surface-strong)] px-3 py-2.5">
            <p className="text-sm leading-6 text-[var(--glass-text-secondary)]">
              &ldquo;{panelData.sourceText}&rdquo;
            </p>
          </div>
        </GlassField>
      ) : null}
        ...
        >
          {panelData.location ? (
            <div className="flex flex-wrap gap-1.5">
              <GlassChip tone="success" onRemove={onRemoveLocation}>
                {panelData.location}
              </GlassChip>
            </div>
          ) : (
