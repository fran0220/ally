 ...
 * Custom dropdown: top section for model selection, divider, bottom section for params.
 * Trigger displays model name + provider + param summary.
 */

import { useCallback, useEffect, useLayoutEffect, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';
import type { CapabilityValue } from '../../../lib/model-config-contract';
import { AppIcon, RatioPreviewIcon } from '../icons';
...
export function ModelCapabilityDropdown({
  ...
  const resolveCapabilityLabel = useCallback((field: CapabilityFieldDefinition): string => {
    const key = `capability.${field.field}`;
    const translated = tv(key);
    // If react-i18next returns the key itself, fall back to field.label
