...
export default function VideoRenderPanel({
  ...
  const aspectRatioConfig = ASPECT_RATIO_CONFIGS[videoRatio] ?? ASPECT_RATIO_CONFIGS['16:9']!

  return (
    <>
      <div className={`grid gap-4 ${aspectRatioConfig.isVertical
        ...
        {allPanels.map((panel, idx) => {
          ...
          const prevPanel = idx > 0 ? (allPanels[idx - 1] ?? null) : null
          ...
          return (
            <div
              ...
              className={`transition-all duration-500 ${highlightedPanelKey === panelKey
                ? 'ring-4 ring-[var(--glass-stroke-focus)] ring-offset-2 ring-offset-[var(--glass-bg-canvas)] rounded-[var(--glass-radius-xl)] scale-[1.02]'
                : ''
              }`}
            >
              <VideoPanelCard
