...
export default function ImageSectionCandidateMode({
  ...
  const safeSelectedIndex = Math.min(Math.max(candidateData.selectedIndex, 0), validCandidates.length - 1)
  const selectedCandidate = validCandidates[safeSelectedIndex]
  ...
  return (
    <div className="w-full h-full relative">
      <MediaImageWithLoading
        ...
        onClick={() => onPreviewImage?.(selectedCandidate)}
          ...
          <div className="flex gap-1">
            ...
            <button
              onClick={async () => {
                ...
                _ulogInfo('[ImageSection] 选中的图片 URL:', selectedCandidate)
                setIsConfirming(true)
                try {
                  await onConfirmCandidate(panelId, selectedCandidate)
