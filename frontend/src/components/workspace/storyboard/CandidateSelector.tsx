...
export default function CandidateSelector({
  ...
  const [w = 16, h = 9] = videoRatio.split(':').map(Number)
  const ratioWidth = Number.isFinite(w) && w > 0 ? w : 16
  const ratioHeight = Number.isFinite(h) && h > 0 ? h : 9
  const thumbWidth = 120
  const thumbHeight = Math.round((thumbWidth * ratioHeight) / ratioWidth)
  return (
