...
export function createManualKeyBaseline(
  ...
  if (characterMatch) {
    ...
    const appearanceIndexRaw = characterMatch[2]
    const suffix = characterMatch[3]
    if (!characterId || !appearanceIndexRaw || !suffix) return null
  ...
  if (locationMatch) {
    ...
    if (!locationId || !suffix) return null
...
export function shouldResolveManualKey(
  ...
  if (characterMatch) {
    const characterId = characterMatch[1]
    const appearanceIndexRaw = characterMatch[2]
    if (!characterId || !appearanceIndexRaw) return true
    const appearanceIndex = Number.parseInt(appearanceIndexRaw, 10)
    const character = characters.find((item) => item.id === characterId)
    const appearance = character?.appearances?.find((item) => item.appearanceIndex === appearanceIndex)
  ...
  if (locationMatch) {
    const locationId = locationMatch[1]
    const suffix = locationMatch[2]
    if (!locationId || !suffix) return true
    const location = locations.find((item) => item.id === locationId)
