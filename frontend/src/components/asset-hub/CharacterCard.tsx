...
export function CharacterCard({ character, onImageClick, onImageEdit, onVoiceDesign, onEdit, onVoiceSelect }: CharacterCardProps) {
    ...
    // 计算属性
    const appearance = character.appearances[activeAppearance] || character.appearances[0] || null
    if (!appearance) {
        return (
            <div className="glass-surface p-3">
                <h3 className="text-sm font-medium text-[var(--glass-text-primary)]">{character.name}</h3>
                <p className="mt-2 text-xs text-[var(--glass-text-tertiary)]">{t('emptyState')}</p>
                <VoiceSettings
