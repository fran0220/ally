...
export function useVideoFirstLastFrameFlow({
  ...
  useEffect(() => {
    const firstModelOption = firstLastFrameModelOptions[0]
    if (!flModel && firstModelOption) {
      setFlModel(firstModelOption.value)
      return
    }
  ...
  const setFlCapabilityValue = useCallback((field: string, rawValue: string) => {
    ...
    const firstOption = definitionField.options[0]
    if (firstOption === undefined) return
    const parsedValue = parseByOptionType(rawValue, firstOption)
  ...
  const getNextPanel = useCallback((currentIndex: number): VideoPanel | null => {
    ...
    return allPanels[currentIndex + 1] ?? null
  }, [allPanels])
