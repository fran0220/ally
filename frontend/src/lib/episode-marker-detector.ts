...
function getMatchCapture(match: RegExpMatchArray, index: number): string {
    const captured = match[index]
    return typeof captured === 'string' ? captured : ''
}

function parseCapturedInt(match: RegExpMatchArray, index: number): number {
    const parsed = parseInt(getMatchCapture(match, index), 10)
...
const DETECTION_PATTERNS: DetectionPattern[] = [
    // 1. 中文"第X集"
    {
        ...
        typeKey: 'episode',
        typeName: '第X集',
        extractNumber: (match) => chineseToNumber(getMatchCapture(match, 1)),
        extractTitle: (match) => getMatchCapture(match, 2).trim() || ''
    },
    // 2. 中文"第X章"
    {
    ...
    // 4. 场景编号 X-Y【场景】 - 只取第一个数字作为集数
    {
        ...
        typeKey: 'scene',
        typeName: 'X-Y【场景】',
        extractNumber: (match) => parseCapturedInt(match, 1),
        extractTitle: (match) => getMatchCapture(match, 2).trim() || ''
    },
    // 5. 数字前缀 "1. 标题" 或 "1、标题"
    {
        ...
        typeName: '数字编号',
        extractNumber: (match) => parseCapturedInt(match, 1),
        extractTitle: (match) => getMatchCapture(match, 2).trim().slice(0, 20) || ''
    },
...
export function detectEpisodeMarkers(content: string): EpisodeMarkerResult {
    ...
    const avgDistance = result.matches.length > 1 && firstMatch && lastMatch
        ? (lastMatch.index - firstMatch.index) / (result.matches.length - 1)
        : 0
    ...
    result.matches.forEach((match, idx) => {
        ...
        const nextMatch = result.matches[idx + 1]
