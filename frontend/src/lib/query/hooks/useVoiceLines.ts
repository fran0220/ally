...
export function useVoiceLines(episodeId: string | null) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetchWithAuth(`/api/novel-promotion/episodes/${episodeId}/voice-lines`)
...
export function useMatchedVoiceLines(projectId: string | null, episodeId: string | null) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetchWithAuth(`/api/novel-promotion/${projectId}/voice-lines?episodeId=${episodeId}`)
...
export function useGenerateVoice(projectId: string | null, episodeId: string | null) {
    ...
    return useMutation({
        mutationFn: async ({ lineId }: { lineId: string }) => {
            ...
            const res = await fetchWithAuth(`/api/novel-promotion/${projectId}/generate-voice`, {
                method: 'POST',
...
export function useBatchGenerateVoices(projectId: string | null, episodeId: string | null) {
    ...
    return useMutation({
        mutationFn: async ({ lineIds }: { lineIds: string[] }) => {
            if (!projectId) throw new Error('Project ID is required')
            const res = await fetchWithAuth(`/api/novel-promotion/${projectId}/batch-generate-voices`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
export function useUpdateVoiceText(episodeId: string | null) {
    ...
    return useMutation({
        mutationFn: async ({ lineId, text }: { lineId: string; text: string }) => {
            const res = await fetchWithAuth(`/api/novel-promotion/voice-lines/${lineId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
