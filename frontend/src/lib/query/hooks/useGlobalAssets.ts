...
export function useGlobalCharacters(folderId?: string | null) {
    const charactersQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetchWithAuth(`/api/asset-hub/characters?${params}`)
...
export function useGlobalLocations(folderId?: string | null) {
    const locationsQuery = useQuery({
        ...
        queryFn: async () => {
            const params = new URLSearchParams()
            if (folderId) params.set('folderId', folderId)
            const res = await fetchWithAuth(`/api/asset-hub/locations?${params}`)
...
export function useGlobalVoices(folderId?: string | null) {
    return useQuery({
        ...
        queryFn: async () => {
            const params = new URLSearchParams()
            if (folderId) params.set('folderId', folderId)
            const res = await fetchWithAuth(`/api/asset-hub/voices?${params}`)
...
export function useGlobalFolders() {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await fetchWithAuth('/api/asset-hub/folders')
...
export function useDeleteFolder() {
    ...
    return useMutation({
        mutationFn: async ({ folderId }: { folderId: string }) => {
            const res = await fetchWithAuth(`/api/asset-hub/folders?folderId=${folderId}`, {
                method: 'DELETE',
            })
            if (!res.ok) {
                const error = await res.json()
