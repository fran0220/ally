...
async function fetchTargetStatesChunk(
  ...
  const response = await fetchWithAuth('/api/task-target-states', {
    method: 'POST',
...
async function flushTaskTargetStateBatch(projectId: string) {
  ...
  try {
    ...
    for (const [chunkIdx, chunkTargets] of chunks.entries()) {
      ...
      for (let i = 0; i < chunkTargets.length; i++) {
        ...
        byTargetQueryKey.set(targetQueryKey(target), state)
      }
    }
