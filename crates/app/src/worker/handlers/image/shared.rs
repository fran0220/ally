...
use sqlx::FromRow;
use waoowaoo_core::{
    api_config::get_system_default_models,
    errors::AppError,
...
fn resolve_model_with_fallback(
    ...
    normalize_model_key(primary)
        .or_else(|| normalize_model_key(secondary))
        .or_else(|| normalize_model_key(Some(system_default)))
}
...
pub async fn get_project_models(
    ...
    let user_pref = sqlx::query_as::<_, UserModelRow>(
        "SELECT analysisModel, characterModel, locationModel, storyboardModel, editModel, videoModel, lipSyncModel FROM user_preferences WHERE userId = ? LIMIT 1",
    )
    .bind(user_id)
    .fetch_optional(mysql)
    .await?;
    ...
    Ok(ProjectModels {
        analysis_model: resolve_model_with_fallback(
            project.analysis_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.analysis_model.as_deref()),
            &system_defaults.analysis_model,
        ),
        character_model: resolve_model_with_fallback(
            project.character_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.character_model.as_deref()),
            &system_defaults.character_model,
        ),
        location_model: resolve_model_with_fallback(
            project.location_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.location_model.as_deref()),
            &system_defaults.location_model,
        ),
        storyboard_model: resolve_model_with_fallback(
            project.storyboard_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.storyboard_model.as_deref()),
            &system_defaults.storyboard_model,
        ),
        edit_model: resolve_model_with_fallback(
            project.edit_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.edit_model.as_deref()),
            &system_defaults.edit_model,
        ),
        video_model: resolve_model_with_fallback(
            project.video_model.as_deref(),
            user_pref
                .as_ref()
                .and_then(|item| item.video_model.as_deref()),
            &system_defaults.video_model,
        ),
...
pub async fn get_user_models(user_id: &str) -> Result<UserModels, AppError> {
    ...
    Ok(UserModels {
        analysis_model: resolve_model_with_fallback(
            row.analysis_model.as_deref(),
            None,
            &system_defaults.analysis_model,
        ),
        character_model: resolve_model_with_fallback(
            row.character_model.as_deref(),
            None,
            &system_defaults.character_model,
        ),
        location_model: resolve_model_with_fallback(
            row.location_model.as_deref(),
            None,
            &system_defaults.location_model,
        ),
        storyboard_model: resolve_model_with_fallback(
            row.storyboard_model.as_deref(),
            None,
            &system_defaults.storyboard_model,
        ),
        edit_model: resolve_model_with_fallback(
            row.edit_model.as_deref(),
            None,
            &system_defaults.edit_model,
        ),
        video_model: resolve_model_with_fallback(
            row.video_model.as_deref(),
            None,
            &system_defaults.video_model,
        ),
        lip_sync_model: resolve_model_with_fallback(
            row.lip_sync_model.as_deref(),
            None,
            &system_defaults.lip_sync_model,
        ),
    })
}
